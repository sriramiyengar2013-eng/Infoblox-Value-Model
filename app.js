...
function buildAssistantSummary() {
  ...
  const infobloxHours = parseFloat(infobloxHoursPerIp.value) || 0;
  const productName = infobloxProduct.value;
  const totalNoInfoblox = calculateAnnualLabor(hourlyRate, parseFloat(manualHoursPerIp.value) || 0, ips) + events * incidentCost;
  const totalInfoblox = calculateAnnualLabor(hourlyRate, infobloxHours, ips) + events * incidentCost * (infobloxProducts[productName]?.incidentFactor || 0.55);
  const savings = totalNoInfoblox - totalInfoblox;
  return `With ${ips.toLocaleString()} managed IPs and ${events} incidents per year, ${productName} is estimated to save ${formatMoney(savings)} annually versus manual operations.`;
}
